...
![](https://cubox.pro/c/filters:no_upscale()?imageUrl=https%3A%2F%2Fcdn.tobebetterjavaer.com%2Fstutymore%2Fclass-load-20231031202641.png&valid=true) 类加载过程
...
class ClassLoader {
    // 1. 通过全限定名获取二进制字节流
    byte[] bytecode = getClassBytecode("com.example.MyClass");
    
    // 2. 将字节流转化为方法区的运行时数据结构
    Class<?> clazz = defineClass(bytecode);
    
    // 3. 在堆中生成Class对象，作为访问入口
    return clazz;
}
```
...
- **方法区/元空间**：存储类的元数据（类结构信息、常量池、字段描述、方法描述）
- **堆内存**：创建对应的java.lang.Class对象，作为访问方法区数据的入口
- **运行时常量池**：从字节码常量池转换而来，位于方法区
...
- **文件格式验证**：魔数、版本号、常量池等
- **元数据验证**：语义分析，如是否有父类、是否继承了final类等
- **字节码验证**：验证程序语义是否合法
- **符号引用验证**：验证符号引用是否能找到对应的类

### 3）Preparation（准备）

JVM 会在该阶段对**类变量**（也称为[静态变量](https://javabetter.cn/oo/static.html)，`static` 关键字修饰的）分配内存并初始化，对应数据类型的默认初始值，如 0、0L、null、false 等。

- 只分配**类变量（static变量）**的内存
- 设置**零值**（0、false、null等）
- **final static**常量会直接赋真实值

也就是说，假如有这样一段代码：

```java
public String chenmo = "wangwu";//不分配内存
public static String wanger = "lisi";//null
public static final String cmower = "zhangsan";//zhangsan
```
...
下面通过一张简化的图来描述它们的区别：
![](https://raw.githubusercontent.com/Ze-d/blog-img/main/java_symbolcite_directcite.png))
...
换句话说，**初始化阶段是执行类构造器方法**（[javap](https://javabetter.cn/jvm/bytecode.html) 中看到的 `<clinit>()` 方法）的过程。

上面这段话可能说得很抽象，不好理解，我来举个例子。

```java
String cmower = new String("wangwu");
```
...
聊完类加载过程，就不得不聊聊类加载器。
![](https://raw.githubusercontent.com/Ze-d/blog-img/refs/heads/main/java_class_loader.png)
...
public class ClassLoaderDemo {
    public static void main(String[] args) {
        ...
        System.out.println("当前类加载器: " + loader1); // AppClassLoader
        
        // 2. 获取父加载器
        ClassLoader parent = loader1.getParent();
        System.out.println("父加载器: " + parent); // ExtClassLoader
        
        // 3. 获取父加载器的父加载器（Bootstrap）
        ClassLoader grandParent = parent.getParent();
        System.out.println("祖父加载器: " + grandParent); // null
        
        // 4. 核心类的加载器
        ClassLoader stringLoader = String.class.getClassLoader();
        System.out.println("String类的加载器: " + stringLoader); // null（Bootstrap）
    }
}
```
